...
def get_img_norm_cfg(dataset_name: str) -> Dict[str, float]:
    ...
    norm_configs = {
        ...
        'IRSTDID-SKY': {'mean': 80.06156125, 'std': 44.45834291286324},
...
def center_crop_or_pad(img: np.ndarray, mask: np.ndarray, patch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    """Deterministic center crop for validation, with padding if needed."""
    h, w = img.shape[:2]
    if h < patch_size or w < patch_size:
        pad_h = max(0, patch_size - h)
        pad_w = max(0, patch_size - w)
...
def resize_sctransnet_patch(img: np.ndarray, mask: np.ndarray, patch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    """Deterministic full-image resize for SCTransNet-style validation."""
    img_t = torch.from_numpy(np.ascontiguousarray(img))[None, None].float()
    mask_t = torch.from_numpy(np.ascontiguousarray(mask))[None, None].float()
    img_r = F.interpolate(img_t, size=(patch_size, patch_size), mode="bilinear", align_corners=False)
    mask_r = F.interpolate(mask_t, size=(patch_size, patch_size), mode="nearest")
    return img_r[0, 0].numpy(), mask_r[0, 0].numpy()
...
def _resolve_existing_subdir(root: str, preferred: str, aliases: Optional[List[str]] = None) -> str:
    """Resolve a dataset subdirectory with basic case-insensitive fallback.

    This keeps existing defaults (`images` / `masks`) working while also
    supporting datasets that use `Images` / `Masks` on case-sensitive filesystems.
    """
    aliases = aliases or []
    ...
    for cand in candidates:
        matched = entry_map.get(cand.lower())
        if matched is not None:
            path = os.path.join(root, matched)
            if os.path.isdir(path):
                return path
...
class SIRSTDataset(Dataset):
    ...
    def __init__(
        ...
        sc_eval_crop: str = "random",
        # MLLM pre-computed CLIP text features
        mllm_features_path: Optional[str] = None,
    ):
        self.root = root
        self.img_dir = _resolve_existing_subdir(root, img_dir, aliases=["Images", "images"])
        self.mask_dir = _resolve_existing_subdir(root, mask_dir, aliases=["Masks", "masks"])
        self.size = size
        ...
        self.sc_eval_crop = str(sc_eval_crop or "random").lower()
        if self.sc_eval_crop not in ("random", "center", "resize"):
            raise ValueError(f"Unsupported sc_eval_crop={sc_eval_crop!r}; expected random, center, or resize.")
    ...
    def __getitem__(self, idx: int):
        ...
        if self.sctransnet_preproc:
            ...
            # Training keeps SCTransNet random crops. Validation can opt into a
            # deterministic crop/resize protocol for reproducible evaluation.
            if self.augment or self.sc_eval_crop == "random":
                img_np, mask_np = random_crop_with_target(
                    img_np, mask_np, self.size, pos_prob=self.sc_pos_prob
                )
            elif self.sc_eval_crop == "center":
                img_np, mask_np = center_crop_or_pad(img_np, mask_np, self.size)
            elif self.sc_eval_crop == "resize":
                img_np, mask_np = resize_sctransnet_patch(img_np, mask_np, self.size)
            else:
                raise ValueError(f"Unsupported sc_eval_crop={self.sc_eval_crop!r}")
