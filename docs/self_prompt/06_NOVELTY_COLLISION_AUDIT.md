...
| SAM-SPL | 浅层高分辨率feature self-prompt、SAM consult-guide、多尺度mutual calibration、纯图像IRSTD部署 | 首次纯图像IR self-prompt；首次浅层feature prompt | candidate identity/no-object、decoder-side拒绝、mask credit（正文待补） |
...
| PromptPilot | multi-agent点优化、SAM global DSC与局部marginal contribution、remove/restore harmful prompt | 首次prompt credit或agentic point optimization | 离线credit蒸馏到单步image-only candidate query（正文待补） |
...
| 2 TB-Prompt | IP-SAM覆盖前/背景prompt-space gating；EviPrompt/Memory-SAM覆盖正负提示 | 每candidate inner/outer/background pair + independent mask + wrong-ring反事实 | **若只把IP-SAM搬到IR则是** | 否 | IP-SAM+MaskSAM组合风险高 | 局部背景不是负点，而是可校准的候选条件；其非对称作用在tiny target上通过shuffled/wrong background因果验证 | 高 |
| 3 RQ-Adapt | SPARK-SAM覆盖IR response adaptation；ReSAM/H-SAM覆盖再查询/两阶段；PromptPilot覆盖feedback | 单次candidate refine/reject、离线response proxy→单步部署、Fa-risk控制 | 部分 | 否 | **很可能是SPARK+ReSAM/PromptPilot** | 冻结首轮response是否可预测有害IR candidate，并以一次轻量update实现拒绝 | 高 |
| 4 EviSet | EviPrompt/UR-SAM/AutoPromptSeg覆盖evidence/UQ；MaskSAM覆盖set | 无reference的per-query三态evidence直接门控mask | 若只加evidential loss则是常规迁移 | 否 | EviPrompt+MaskSAM | candidate uncertainty的可校准risk控制是否优于max-score/temperature baseline | 中高 |
| 5 CreditDrop | PromptPilot/AlignSAM/PPD覆盖prompt credit、删除和RL；AoP-SAM覆盖response筛选 | exact drop teacher→single-pass image-only credit head | 部分 | 否 | PromptPilot蒸馏化 | leave-one-out mask delta可作为IR harmful prompt监督，并在单次部署保留收益 | 高 |
...
- **AlignSAM/PromptPilot/PPD**覆盖RL/agent credit与有害prompt删除；Idea3必须是无需在线GT、单步部署的轻量student。
