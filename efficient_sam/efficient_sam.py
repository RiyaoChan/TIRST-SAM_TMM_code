...
class EfficientSam(nn.Module):
    ...
    def __init__(
        ...
        use_clutter_prompt_decoder: bool = False,
        clutter_gate_beta: float = 0.3,
    ) -> None:
        """
        ...
        self.use_clutter_prompt_decoder = bool(use_clutter_prompt_decoder)
        self.clutter_gate_beta = float(clutter_gate_beta)
        try:
        ...
        if self.use_clutter_prompt_decoder:
            clutter_mid_dim = max(1, neck_dim // 4)
            self.clutter_head = nn.Sequential(
                nn.Conv2d(neck_dim, clutter_mid_dim, kernel_size=3, padding=1, bias=False),
                nn.GroupNorm(1, clutter_mid_dim),
                nn.GELU(),
                nn.Conv2d(clutter_mid_dim, 1, kernel_size=1),
            )
            self.clutter_prompt_proj = nn.Sequential(
                nn.Conv2d(1, neck_dim, kernel_size=1, bias=False),
                nn.GroupNorm(1, neck_dim),
            )
        else:
            self.clutter_head = None
            self.clutter_prompt_proj = None
    ...
    def predict_masks(
        ...
        if self.use_clutter_prompt_decoder and self.clutter_head is not None and self.clutter_prompt_proj is not None:
            clutter_logits = self.clutter_head(image_embeddings)
            clutter_prob = torch.sigmoid(clutter_logits)
            clutter_gate = 1.0 - self.clutter_gate_beta * clutter_prob
            clutter_prompt = self.clutter_prompt_proj(clutter_prob)
            image_embeddings = image_embeddings * clutter_gate + clutter_prompt
            if return_aux:
                aux_dict["clutter_logits"] = clutter_logits
                aux_dict["clutter_prob"] = clutter_prob
        ...
        if return_aux:
            return output_masks, iou_predictions, aux_dict
...
def build_efficient_sam(
    ...
    sam = EfficientSam(
        ...
        use_clutter_prompt_decoder=use_clutter_prompt_decoder,
        clutter_gate_beta=clutter_gate_beta,
    )
